...
const isOwner = computed(() =>
  me.value !== null && property.value !== null &&
  property.value.user_id === me.value.id
)
...
onMounted(async () => {
  ...
  if (token) {
    ...
    } catch (error) { console.error('[TheExib] carregar usuário', error) }
  }
})
...
async function handleFav() {
  if (!logado.value) { router.push('/login'); return }
  ...
  <div class="all">
    ...
    <div class="home-details">
      ...
      <div class="home-informations">
        <div class="ende">
          <div class="casa-endereco"><p>{{ property?.local }}</p></div>
          ...
          <div class="card-contato" @click="copiarContato" :class="{ copiado }">
            <div
              ...
              :style="property?.user?.profile_picture
                ...
                : {}"
            >
              <span v-if="!property?.user?.profile_picture">{{ property?.user?.name?.charAt(0) }}</span>
            </div>
            <p @click.stop="verPerfil">{{ property?.user?.name }} {{ property?.user?.last_name }}</p>
...
.btn-deletar:disabled { opacity: 0.6; cursor: not-allowed; }

.home-details { width: 100%; display: flex; align-items: flex-start; justify-content: space-between; }
...
.home-informations .ende { width: 100%; min-height: 60px; display: flex; justify-content: space-between; align-items: center; }
.ende p { margin: 0; }
...
.icon-fav { color: var(--color-icon-inactive); width: clamp(35px, 2.5vw, 40px); height: clamp(35px, 2.5vw, 40px); }

.fav.ativo .icon-fav { color: var(--color-primary); }
.icon-fav { transition: color 0.2s; }

.casa-endereco { width: 80%; display: flex; align-items: center; font-size: clamp(1.6rem, 1.5vw, 2.2rem); font-weight: 600; }
...
.info .divisoria { height: 3rem; flex-shrink: 0; border-radius: 20px; width: 1px; background-color: var(--color-border-black); }

.home-informations .simulation { width: 100%; min-height: 70px; font-size: clamp(1.1rem, 1.1vw, 1.8rem); text-wrap: nowrap; }
.slider { width: 100%; }
...
.juntar { display: flex; flex-direction: column; position: relative; }
.n-baixo { margin: 0; position: absolute; right: 0; top: 70%; font-size: clamp(1rem, 1vw, 1.5rem); opacity: 0.6; }

.home-informations .contact { width: 100%; gap: 10px; display: flex; flex-direction: column; position: relative; }
.contato-escrita { min-height: 30px; display: flex; align-items: center; font-size: clamp(1rem, 1vw, 1.6rem); }

.card-contato {
  width: 100%; min-height: 60px;
  display: flex; justify-content: space-around; align-items: center;
  font-size: clamp(1.1rem, 1.05vw, 1.6rem); font-weight: 500;
  border-radius: 15px; box-shadow: var(--shadow-sm);
  cursor: pointer; transition: box-shadow 0.3s, background-color 0.3s;
}
.card-contato:hover { box-shadow: var(--shadow-hover-blue); }
.card-contato.copiado { background-color: #e8f5e9; }

.card-contato .img { height: 3rem; aspect-ratio: 1/1; border-radius: 50%; background-position: center; background-size: cover; cursor: pointer; }
.card-contato .img.semFoto { background-color: #cfcfcf; display: flex; align-items: center; justify-content: center; color: #666; font-weight: 600; }
.card-contato p { cursor: pointer; margin: 0; }

.phone-icon-wrap { width: clamp(1.8rem, 1vw, 2.2rem); height: clamp(1.8rem, 1vw, 2.2rem); display: flex; align-items: center; justify-content: center; transition: color 0.3s; }
...
.toast {
  ...
  background: #1a2fa8; color: white; padding: 8px 18px; border-radius: 20px;
...
.btn-solicitar {
  ...
  background-color: var(--color-primary); border-radius: 20px; border: none;
  color: white; font-family: "Poppins", sans-serif;
  font-size: clamp(1.1rem, 1.15vw, 30px); font-weight: 600;
  cursor: pointer; transition: background-color 0.3s;
}
.btn-solicitar:hover { background-color: var(--color-primary-hover); }

@media (max-width: 768px) {
  .all { width: 95%; margin-top: 7vw; margin-bottom: 7vw; }
  .home-title { flex-wrap: wrap; }
  .gerenciar-btns { width: 100%; }
  .home-details { flex-direction: column; gap: 15px; }
  .home-photo { width: 100%; aspect-ratio: 16/9; }
  .home-informations { width: 100%; gap: 40px; padding-bottom: 40px; padding-top: 40px; }
}
</style>
